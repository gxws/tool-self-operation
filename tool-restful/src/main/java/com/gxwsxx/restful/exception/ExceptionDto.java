...
public class ExceptionDto extends Exception {
	...
	public String getMessage() {
		ExceptionLabel ed = this.getClass().getAnnotation(ExceptionLabel.class);
		if (null == ed) {
	...
	private Field[] fields(Class<?> cls) {
		...
		if (scls.equals(ExceptionDto.class)) {
			return selffs;
		} else {
	...
	private String field2String(Field[] fields) {
		...
		Method gm = null;
		for (Field f : fields) {
			ExceptionLabel ed = f.getAnnotation(ExceptionLabel.class);
			if (null == ed) {
				continue;
			}
			key = ed.value();
